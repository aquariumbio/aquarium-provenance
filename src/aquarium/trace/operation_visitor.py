...
class PassthruOperationVisitor(OperationProvenanceVisitor):
    ...
    """
    @abc.abstractmethod
    def __init__(self, trace: ProvenanceTrace, name: str):
...
class MeasurementVisitor(OperationProvenanceVisitor):
    ...
    accuri_channels = ['FL1-A', 'FL2-A', 'FL4-A', 'FSC-A', 'SSC-A']
...
class PlateReaderMeasurementVisitor(
    ...
    def visit_trace(self, trace: ProvenanceTrace):
        ...
        visitors.
        """
        upload = self.get_calibration_upload(trace)
        filename = None
        if upload:
    ...
    def get_calibration_upload(self, plan: ProvenanceTrace):
...
def create_operation_visitor():
    """
    Creates visitor to apply heuristic fixes to a ProvenanceTrace object.
